...
This Helm chart installs and configures the [OneNet](https://github.com/l1feai/one-network) services within a Kubernetes cluster. The chart includes the management, signal, and relay components of OneNet, providing secure peer-to-peer network connections across various environments.
...
To install the chart with the release name `onenet`:

```bash
helm repo add onenet https://l1feai.github.io/helms
helm install onenet l1feai/one-network
```

You can override default values by specifying a `values.yaml` file:

```bash
helm install onenet l1feai/one-network -f values.yaml
```

### Uninstalling the Chart

To uninstall/delete the `onenet` release:

```bash
helm uninstall onenet
```
...
The following table lists the configurable parameters of the OneNet Helm chart and their default values.
...
| dashboard.image.repository | string | `"l1feai/dashboard"` |  |
...
| management.image.repository | string | `"l1feai/one-network"` |  |
...
| management.podCommand.args[4] | string | `"--single-account-mode-domain=onenet.selfhosted"` |  |
| management.podCommand.args[5] | string | `"--dns-domain=onenet.selfhosted"` |  |
...
| relay.image.repository | string | `"l1feai/relay"` |  |
...
| signal.image.repository | string | `"l1feai/signal"` |  |
